...
import math
from layer import GraphClassifier
...
if __name__ == '__main__':
    ...
    for fold in range(n_folds):
        ...
        print("Labels range after adjustment:", labels.min().item(), "to", labels.max().item())
        model = GraphClassifier(
            node_in_dim=118,
            local_hidden_dim=64,
            hierarchical_in_dim=2140,
            hierarchical_out_dim=2,
            nonimg_data=nonimg,
            phonetic_data=phonetic_score
        ).to(opt.device)
        print(model)
        ...
        def train():
            ...
            print("\r\n => Fold {} test accuracy {:.5f}".format(fold, acc))
